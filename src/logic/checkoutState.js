...
const qtyOf = (c) => { const q = Number(c && c.qty); return Number.isFinite(q) && q > 0 ? q : 0; };
...
export function itemCounts(events, { todayKey, dayOf } = {}) {
  ...
  for (const [eqId, it] of Object.entries(per)) {
    ...
    out[eqId] = {
      ...
      owner: it.lastPick ? { employeeId: it.lastPick.employeeId, employeeName: it.lastPick.employeeName, ts: it.lastPick.ts } : null,
